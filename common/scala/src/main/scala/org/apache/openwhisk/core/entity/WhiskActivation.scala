...
case class WhiskActivation(namespace: EntityPath,
  ...
  def withLogs(logs: ActivationLogs, init: String = "cold", userTime: String = "0", langTime: String = "0", bareTime: String = "0") = 
    // Explicitly log details of the activation
    copy(logs = ActivationLogs(Vector(init, userTime, langTime, bareTime) ++: logs.logs)).revision[WhiskActivation](rev) 

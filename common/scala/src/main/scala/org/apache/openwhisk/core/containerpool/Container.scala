...
trait Container {
  ...
  /** Update the container with given cpu-shares and memory. */
  ...
  /** Clean code in the user level container, which then degrades to lang level. */
  def clean(parameters: JsObject,
            timeout: FiniteDuration,
            maxConcurrent: Int,
            entity: Option[WhiskAction] = None)(implicit transid: TransactionId): Future[Interval] = {
    val start = transid.started(
      this,
      LoggingMarkers.INVOKER_ACTIVATION_CLEAN,
      s"sending clean to $id $addr",
      logLevel = InfoLevel)
    val body = JsObject("value" -> parameters)
    callContainer("/clean", body, timeout, maxConcurrent, retry = true)
      .andThen { // never fails
        case Success(r: RunResult) =>
          transid.finished(
            this,
            start.copy(start = r.interval.start),
            s"clean result: ${r.toBriefString}",
            endTime = r.interval.end,
            logLevel = InfoLevel)
        case Failure(t) =>
          transid.failed(this, start, s"clean failed with $t")
      }
      .flatMap { result =>
        if (result.ok) {
          Future.successful(result.interval)
          ...
          Future.failed(
            CleanError(
              result.interval,
              s"clean $id timeout after $timeout"))
        } else {
          Future.failed(
            CleanError(
              result.interval,
              s"clean $id failed with unknown reason"))
        }
      }
  }
...
/** Indicates an error while cleaning a container */
case class CleanError(interval: Interval, msg: String) extends Exception(msg)

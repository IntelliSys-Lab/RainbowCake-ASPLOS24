...
case class ContainerPoolConfig(userMemory: ByteSize,
  ...
  // private val totalShare = 8.0 // In experiments, all invoker servers have homogeneous hardwares (i.e., 8 CPU cores and 2048 MBs)
  ...
  def cpuShare(reservedMemory: ByteSize) =
    ...
    // max((totalShare / (userMemory.toBytes / reservedMemory.toBytes)).toInt, 1) // The minimum CPU core for each function is 1
}

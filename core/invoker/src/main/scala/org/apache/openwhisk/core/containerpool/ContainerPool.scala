...
case class PrewarmContainer(action: ExecutableWhiskAction)
...
class ContainerPool(childFactory: ActorRefFactory => ActorRef,
  ...
  // Invocation history parameters (seconds)
  val alpha: Double = 0.996
  val quantile: Double = 0.8
  val prewarmMin: Int = 1
  val prewarmMax: Int = 60*10
  val aliveMin: Int = 60*1
  val aliveMax: Int = 60*10
  val historyTimeout: Int = 60*60*24
  val historyLength: Int = 6

  // History of three levels
  var userHistory = immutable.Map.empty[ExecutableWhiskAction, Distribution]
  var langHistory = immutable.Map.empty[String, Distribution]
  var bareHistory = new Distribution(
  ...
  var actionHistory = immutable.Map.empty[ExecutableWhiskAction, History]
  var prewarmEvents = immutable.Vector.empty[Cancellable]
  ...
  def receive: Receive = {
    ...
    case r: Run =>
      if (r.action.name.name.contains("EndExperiment")) {
        prewarmEvents.foreach(_.cancel())
        freePool.foreach { x => 
          removeContainer(x._1)
        }
      }

      val arrivalTime: Long = Instant.now.toEpochMilli
      logging.info(this, s"Job ${r.toString} arrived at ${arrivalTime.toString}")

      // Check if the message is resent from the buffer. Only the first message on the buffer can be resent.
      val isResentFromBuffer = runBuffer.nonEmpty && runBuffer.dequeueOption.exists(_._1.msg == r.msg)

      // Update historical information upon seeing a new request
      if (!isResentFromBuffer && !r.action.name.name.contains("invokerHealthTest")) {
        val action = r.action
        val kind = action.exec.kind

        // Update User level history
        val userTs = userHistory.get(action) match {
          case Some(dist) => dist.update(arrivalTime)
          case None => {
            val dist = new Distribution(
              historyTimeout=historyTimeout,
              historyLength=historyLength
            )
            userHistory = userHistory + (action -> dist)
            dist.update(arrivalTime)
          }
        }
        logging.info(this, s"Update job ${r.toString} in User history with time series ${userTs.toString}")

        // Update Lang level history
        val langTs = langHistory.get(kind) match {
          case Some(dist) => dist.update(arrivalTime)
          case None => {
            val dist = new Distribution(
              historyTimeout=historyTimeout,
            ...
            langHistory = langHistory + (kind -> dist)
            dist.update(arrivalTime)
          }
        }
        logging.info(this, s"Update job ${r.toString} in Lang history with time series ${langTs.toString}")

        // Update Bare level history
        val bareTs = bareHistory.update(arrivalTime)
        logging.info(this, s"Update job ${r.toString} in Bare history with time series ${bareTs.toString}")

        // Update activation message for prewarming only once
      ...
      // Fire a pre-warming event if not the end signal of experiments or invoker health test action
      if (!r.action.name.name.contains("invokerHealthTest") && !r.action.name.name.contains("EndExperiment")) {
        val dist = userHistory.get(r.action) match {
          case Some(dist) => dist
          case None => {
            val dist = new Distribution(
              historyTimeout=historyTimeout,
            ...
            userHistory = userHistory + (r.action -> dist)
            dist
          }
        }
        val iat: Int = dist.predictPrewarm(
          quantile=quantile, 
          min=prewarmMin, 
          max=prewarmMax
        )
        logging.info(this, s"Schedule prewarm event: action ${r.action} in ${iat} seconds")
        val cancellable = context.system.scheduler.scheduleOnce(iat.seconds, self, PrewarmContainer(r.action))
        prewarmEvents = prewarmEvents :+ cancellable
      }
      ...
      if (runBuffer.isEmpty || isResentFromBuffer) {
        ...
        logging.info(this, s"Receive job ${r.toString} while free pool is ${freePool.toString}")
        ...
        val createdContainer =
          ...
          ContainerPool
            ...
            }) // User, Lang and Bare containers always know their states
            .orElse {
              logging.info(this, s"No user/lang/bare containers, create new container")
              // There was no User/Lang/Bare container. Try to take a prewarm container or a cold container.
    ...
    // Container is either warmed or in user level, and free to take more work
    case NeedWork(warmData: WarmedData) =>
      val oldData = freePool.get(sender()).getOrElse(busyPool(sender()))
      val newData = if (oldData.activeActivationCount >= 1) {
        warmData.copy(lastUsed = oldData.lastUsed, activeActivationCount = oldData.activeActivationCount - 1)
      } else {
        warmData.copy(lastUsed = oldData.lastUsed, activeActivationCount = 0)
      }
      
      // Predict next alive time for a user level container
      if (warmData.aliveState == "user" && !warmData.action.name.name.contains("invokerHealthTest")) {
        val h = actionHistory.get(warmData.action) match {
          case Some(h) => {
            h.updateMemory(warmData.container.catMemory()(TransactionId.invokerNanny), "user")
            h
          }
          case None => {
            val h = new History(warmData.action)
            actionHistory = actionHistory + (warmData.action -> h)
            h.updateMemory(warmData.container.catMemory()(TransactionId.invokerNanny), "user")
            h
          }
        }

        val dist = userHistory.get(warmData.action) match {
          case Some(dist) => dist
          case None => {
            val dist = new Distribution(
              historyTimeout=historyTimeout,
            ...
            userHistory = userHistory + (warmData.action -> dist)
            dist
          }
        }
        val upperBound: Int = h.upperBound(alpha=alpha, level="user", default=aliveMax)
        sender() ! UpdateTimeout(
          dist.predictAlive(
            quantile=quantile, 
            min=aliveMin, 
            max=upperBound
        ...
        logging.info(this, s"${newData} state user with upper bound of ${upperBound}")
      }
    ...
    // Container is either in lang level or prewarmed, and ready to take work
    case NeedWork(data: PreWarmedData) =>
      if (data.aliveState == "lang") { // Alive lang level container
        val oldData = freePool.get(sender()).getOrElse(busyPool(sender()))
        val newData = data.copy(lastUsed = data.lastUsed)

        data.action match {
          case Some(action) => {
            val h = actionHistory.get(action) match {
              case Some(h) => {
                h.updateMemory(data.container.catMemory()(TransactionId.invokerNanny), "lang")
                h
              }
              case None => {
                val h = new History(action)
                actionHistory = actionHistory + (action -> h)
                h.updateMemory(data.container.catMemory()(TransactionId.invokerNanny), "lang")
                h
              }
            }
            // Predict next alive time for a lang level container
            val dist = langHistory.get(data.kind) match {
              case Some(dist) => dist
              case None => {
                val dist = new Distribution(
                  historyTimeout=historyTimeout,
                ...
                langHistory = langHistory + (data.kind -> dist)
                dist
              }
            }
            val upperBound: Int = h.upperBound(alpha=alpha, level="lang", default=aliveMax)
            sender() ! UpdateTimeout(
              dist.predictAlive(
                quantile=quantile, 
                min=aliveMin, 
            ...
            logging.info(this, s"${newData} state lang with upper bound of ${upperBound}")
          }
          case None => 
        }
        ...
        processBufferOrFeed()
      } else { // Normal prewarmed container
        prewarmStartingPool = prewarmStartingPool - sender()
        prewarmedPool = prewarmedPool + (sender() -> data)
      }

    // Container is in Bare level and ready to take work
    case NeedWork(data: MemoryData) =>
      val oldData = freePool.get(sender()).getOrElse(busyPool(sender()))
      val newData = data.copy(lastUsed = oldData.lastUsed)

      data.action match {
          case Some(action) => {
            val h = actionHistory.get(action) match {
              case Some(h) => {
                h.updateMemory(2.0, "bare")
                h
              }
              case None => {
                val h = new History(action)
            ...
            val upperBound: Int = h.upperBound(alpha=alpha, level="bare", default=aliveMax)
            sender() ! UpdateTimeout(
              bareHistory.predictAlive(
                quantile=quantile, 
                min=aliveMin, 
                max=upperBound
              ).seconds
            )
            logging.info(this, s"${newData} state bare with upper bound of ${upperBound}")
          }
          case None => 
        }
    ...
    case ContainerRemoved(replacePrewarm) =>
      ...
      // logging.info(this, s"before remove: free pool ${freePool.toString}, busy pool ${busyPool.toString}")

      freePool.get(sender()).foreach { f =>
        // logging.info(this, s"remove ${sender().toString} from free pool")
        freePool = freePool - sender()
      }

      // container was busy (busy indicates at full capacity), so there is capacity to accept another job request
      busyPool.get(sender()).foreach { _ =>
        // logging.info(this, s"remove ${sender().toString} from busy pool")
        busyPool = busyPool - sender()
      }
      processBufferOrFeed()
      ...
      prewarmedPool.get(sender()).foreach { data =>
        // logging.info(this, s"remove ${sender().toString} from prewarm pool")
      ...
      // logging.info(this, s"after remove: free pool ${freePool.toString}, busy pool ${busyPool.toString}")
    ...
    // case PrewarmContainer(action: ExecutableWhiskAction, job: Run) =>
    //   // First determine composition of containers to be prewarmed
    //   userHistory.foreach{
    //     case (action, history) =>
    //       userPrewarmRun.get(action) match {
    //         case Some(job) => {
    //           val invocationNamespace = job.msg.user.namespace.name
    //           // Count current containers in free pool
    //           val current: Int = freePool.count {
    //             case (_, c @ WarmedData(_, `invocationNamespace`, `action`, _, _, _)) => true
    //             case _ => false
    //           }
    //           // Exclude current idle containers, just fill up the slack
    //           val desired: Int = history.predictPrewarm()
    //           if (current < desired) {
    //             (current until desired).foreach { _ =>
    //               prewarmUserContainer(action, job)
    //             }
    //           }
    //         }
    //         case None =>
    //       }
    //   }
    case PrewarmContainer(action: ExecutableWhiskAction) =>
      userPrewarmRun.get(action) match {
        case Some(job) => {
          val invocationNamespace = job.msg.user.namespace.name
          // Count current containers in free pool
          ...
          // Only prewarm if no idle containers available
          if (current > 0) {
            prewarmUserContainer(action, job)
          }
          // prewarmUserContainer(action, job)
    ...
    case UpdateInitTime(action, userTime, langTime, bareTime) =>
      actionHistory.get(action) match {
        case Some(h) => h.updateTime(userTime, langTime, bareTime)
        case None => {
          val h = new History(action)
          actionHistory = actionHistory + (action -> h)
          h.updateTime(userTime, langTime, bareTime)
        }
      }
      logging.info(this, s"Update init time for action ${action}: userTime ${userTime}, langTime ${langTime}, bareTime ${bareTime}")
  }
  ...
  def createContainer(memoryLimit: ByteSize): (ActorRef, ContainerData) = {
    ...
    logging.info(this, s"add ${ref.toString} to free pool")
    freePool = freePool + (ref -> data)
    ref -> data
  }

  /** Creates a new user container and updates state accordingly. */
...
object ContainerPool {
  ...
  protected[containerpool] def schedule[A](action: ExecutableWhiskAction,
    ...
    // Find a user-level container if any
    idles
      .find {
      ...
      .orElse { // Or find a lang-level container if any
        val kind = action.exec.kind
        idles.find {
          case (_, c @ PreWarmedData(_, `kind`, _, _, _, _)) => true
          case _ => false
        }
      }
      .orElse { // Or find a bare-level container if any
        idles.find {
          case (_, c @ MemoryData(_, _, _)) => true
          case _ => false
        }
      }
  }
  ...
  protected[containerpool] def remove[A](pool: Map[A, ContainerData],
    ...
    val freeContainers = pool.collect {
      // Only three levels of keep-alive containers (User, Lang and Bare) will be removed. 
      // Prewarmed containers will stay always.
      case (ref, d: WarmedData) if d.activeActivationCount <= 0 => ref -> d
      case (ref, d: PreWarmedData) if d.activeActivationCount <= 0 => ref -> d
      case (ref, d: MemoryData) if d.activeActivationCount <= 0 => ref -> d
    }
